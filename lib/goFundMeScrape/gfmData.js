...
async function scrapeGetDescription(url) {
  //this is a delay timer to get around too many requests response
  ...
  const awaitDelayTimer = await delayTimer();
  awaitDelayTimer;
...
async function scrapeThing(url) {
  ...
  const awaitUrlDescriptionObjectPush = await urlAndDescriptionObjectPush();
  awaitUrlDescriptionObjectPush;
  ...
  console.log('end of scrape function');
