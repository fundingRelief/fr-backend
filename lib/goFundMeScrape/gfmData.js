...
//     arr.slice(i * size, i * size + size)
//   );
>>>>>>> da99180cc816df2122e79fdb90c76b770eed79b8
...
// const mergedObject = newObject.map((item) => {
// const combine = {...item, ...item, ...item, ...item, ...item, ...item}
// return combine
// })
...
const titleObj = combinedArray2[0].map((item) => {
  emptyObject.push({ campaign_name: item})
})

array = [[1,1,1],[2,2,2],[3,3,3]];

const thing = array.map((item) => {
  let innerArray = array[item];
  innerArray.map({ campaign_name: item})

})

// const rows = combinedArray2.length;
// for( let i=0; i<rows; i++){
//   let items = combinedArray[i].length;
//     for(let n=0; i < items; n ++){
//       items.push({campaign_name: item})
//     }
// }
=======
// array = [
