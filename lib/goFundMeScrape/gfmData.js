...
const combineArray3 = combinedArray2.concat([percentages])

/*CREATE MAIN ARRAY WITH PERCENTAGES*/
const fundMainArr = [];

combineArray3[0].map((x) => {
  fundMainArr.push({
    campaign_name: x,
    current_amount: null,
    goal: null,
...
const currentAmountObjectPush2 = () => {
  combineArray3[1].map((x, i) => {
    fundMainArr[i].current_amount = x;
  });
};

const goalAmountObjectPush2 = () => {
  combineArray3[2].map((x, i) => {
    fundMainArr[i].goal = x;
  });
};

const locationObjectPush2 = () => {
  combineArray3[3].map((x, i) => {
    fundMainArr[i].location = x;
  });
};

const urlObjectPush2 = () => {
  combineArray3[4].map((x, i) => {
    fundMainArr[i].link_url = x;
  });
};

const descriptionObjectPush2 = () => {
  combineArray3[5].map((x, i) => {
    fundMainArr[i].description = x;
  });
};

const lastDonationObjectPush2 = () => {
  combineArray3[6].map((x, i) => {
    fundMainArr[i].last_donation = x;
  });
};

const imgURLObjectPush2 = () => {
  combineArray3[7].flat().map((x, i) => {
    fundMainArr[i].img_url = x;
  });
};

const percentageObjectPush1 = () => {
  combineArray3[8].map((x, i) => {
    fundMainArr[i].percentage_raised = x;
  });
};

currentAmountObjectPush2()
goalAmountObjectPush2()
locationObjectPush2()
urlObjectPush2()
descriptionObjectPush2()
lastDonationObjectPush2()
imgURLObjectPush2()
percentageObjectPush1()

console.log(fundMainArr);
